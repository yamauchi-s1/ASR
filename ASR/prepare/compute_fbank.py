...
class FeatureExtractor():
    ...
    FBANK(Filter Bank, メルフィルタバンク) :  音声信号の周波数成分をメル周波数軸に基づいて抽出した特徴量

        1. FFT(高速フーリエ変換)を用いて周波数スペクトルを計算
        2. 計算されたスペクトルをメルスケールに基づいた一連の三角フィルタ(メルフィルタバンク)に適用
        3. 各フィルタに対数をとり特徴量を生成
        
    MFCC(Mel-Freqyency Cepstarl Coefficients) : FBANKにさらに処理を加えた特徴量。機械学習において最も広く使われている
    
        1. メルフィルタバンクをを用いてフィルタバンクを生成する
        2. FBANKで得られた対数をDCT(離散コサイン変換)する。これにより周波数成分の情報が時間領域に変換される
        3. 高次のDCT係数をリフトし、音声の動的な特性を強調し、低次の係数の影響を和らげる処理が行われる。
    '''
...
if __name__ == "__main__":
    ...
    train_small_wav_scp = '/workspaces/python/ASR/data/label/train_small/wav.scp'
    train_small_out_dir = '/workspaces/python/ASR/data/fbank/train_small'
    train_large_wav_scp = '/workspaces/python/ASR/data/label/train_large/wav.scp'
    train_large_out_dir = '/workspaces/python/ASR/data/fbank/train_large'
    dev_wav_scp = '/workspaces/python/ASR/data/label/dev/wav.scp'
    dev_out_dir = '/workspaces/python/ASR/data/fbank/dev'
    test_wav_scp = '/workspaces/python/ASR/data/label/test/wav.scp'
    test_out_dir = '/workspaces/python/ASR/data/fbank/test'
